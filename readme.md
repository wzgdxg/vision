...
38. TCP 会在传输层安全自动分段，无任何拦截风险；UDP 超大包会触发底层IP 强制分片（独立数据包），这类分片包会被防火墙 / 路由器 / 运营商直接丢弃，所以 UDP 业务包必须限制 1400 以内，大数据传输由业务层自己手动分片、排序、重组。

> TCP 分段：个个是独立完整 IP 包，安全无拦截  
  IP 分片：全是绑定的残缺碎片，不是独立包，生产禁用  
  传输层自动分段是安全的，完整的IP包，网络层自己分的是不安全的IP分片，是不完整的(只有第一个分片有 TCP/UDP 头部，后面所有分片只有纯数据、没有四层头)。   
  原因就是tcp会协商MSS大小，自动分段，每段独立成完整的报文。udp无 MSS 限制，不控制单包大小，应用层多大，UDP 就原样交给 IP，整体超出二层 MTU 1500ip层一次无法发送这么大，所以拆成了ip分片。  

| 层级 | 名称 | 核心作用 | 核心单位 |
| --- | --- | --- |---|
| 第 1 层 | 物理层 | 网线 / 光纤 / 电波，只传 01 比特流 | 比特        |
| 第 2 层 | 数据链路层（二层） | 局域网转发、MAC 寻址、帧限制 | 二层帧  (以太网帧)   |
| 第 3 层 | 网络层（三层） | 跨网路由、IP 寻址、IP 分片 | IP 数据包    |
| 第 4 层 | 传输层（四层） | 端口通信、TCP/UDP 控制 | TCP/UDP 段 |
| 第 5 层 | 应用层 | 业务数据、HTTP/WebSocket/UDP 自定义协议 | 业务载荷      |

> 上面的是5层模型，osi是7层模型，传输层和应用层间依次有会话层、表示层。但这两层在应用层都会有实现，所以边界模糊，已被放弃 （会话层部分是传输层实现：TCP 连接、四次挥手、Keep-Alive 保活）。

各层数据包大小：

| 层级 | 协议 | 头部大小 | 单包最大业务负载 | 拆分方式 |
| --- | --- | --- | --- | --- |
| 二层 | 以太网 | 14B | 1500(MTU) | 硬件硬限制 |
| 三层 | IPv4 | 20B+ | 1480 | 超了→IP 分片 (危险) |
| 四层 | TCP | 20B+ | 1460(MSS) | 四层自动分段 (安全) |
| 四层 | UDP | 8B | 1472 | 无自动拆分，超了必 IP 分片 |

38. 网络传输依赖mac地址和ip。[网络传输依赖mac地址和ip](./network_work.md)。 
